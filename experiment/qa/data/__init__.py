...
class QAData(experiment.Data):
    ...
    def setup(self, test_split_name, valid_split_name, train_split_name):
        readers = self._get_train_readers()
        self.logger.info('Loading {} train datasets'.format(len(readers)))
        archives = [reader.read(test_split_name, valid_split_name, train_split_name) for reader in readers]
        ...
        if 'embeddings_path' in self.config:
            ...
            # python 3.7
            embeddings_dicts_sizes = [len(next(iter(ed.values()))) for ed in embeddings_dicts]
